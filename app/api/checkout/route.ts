...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let { items, shippingAddress, email, phone, promoCode, userId } = body;

    // Sanitize inputs
    email = sanitizeEmail(email);
    if (phone) phone = sanitizePhone(phone);
    if (shippingAddress?.name)
      shippingAddress.name = sanitizeText(shippingAddress.name);
    if (shippingAddress?.line1)
      shippingAddress.line1 = sanitizeText(shippingAddress.line1);
    if (shippingAddress?.line2)
      shippingAddress.line2 = sanitizeText(shippingAddress.line2);
    if (shippingAddress?.city)
      shippingAddress.city = sanitizeText(shippingAddress.city);
    if (shippingAddress?.state)
      shippingAddress.state = sanitizeText(shippingAddress.state);
    ...
    for (const item of items) {
      const product = trustedProducts.find(
        (p: any) => p._id === item.productId,
      );

      if (!product) {
        log.warn(`Checkout: Product verified failed`, {
          productId: item.productId,
        });
        return NextResponse.json(
          { success: false, error: `Product not available: ${item.name}` },
          { status: 400 },
        );
      }
    ...
    if (promoCode) {
      ...
      if (couponResult.valid) {
        ...
        return NextResponse.json(
          {
            success: false,
            error: `Invalid promo code: ${couponResult.error}`,
          },
          { status: 400 },
        );
      }
    ...
    // Note: All product prices are inclusive of GST — no additional tax is applied
    const shipping = 0; // Free shipping policy
    const tax = 0; // Prices are inclusive of GST
    const total = Math.round(
      Math.max(0, calculatedSubtotal - calculatedDiscount) + shipping,
    );
    ...
    try {
      ...
      log.error("Database error creating order", dbError, {
        orderNumber,
        email,
      });
      return NextResponse.json(
