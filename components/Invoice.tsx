...
const Invoice = forwardRef<HTMLDivElement, InvoiceProps>(({ order }, ref) => {
    ...
    return (
        <div ref={ref} className="bg-white p-8 md:p-12 max-w-4xl mx-auto text-black font-sans" id="invoice-component">
            {/* Header */}
            <div className="flex justify-between items-start mb-12">
                <div>
                    <h1 className="text-3xl font-serif font-bold mb-2">VISHWA</h1>
                    <span className="text-xs tracking-[0.4em] uppercase text-accent-gold block mb-4">
                        Lifestyle
                    </span>
                ...
                <div className="text-right">
                    <h2 className="text-4xl font-light text-gray-200 uppercase mb-4">Invoice</h2>
            ...
            <div className="flex flex-col md:flex-row justify-between mb-12 gap-8">
                <div className="flex-1">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3 block">Bill To</h3>
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                        ...
                        Phone: {order.shipping_address?.phone || order.phone}<br />
                        Email: {order.email}
                    </p>
                </div>
                <div className="flex-1">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-3 block">Ship To</h3>
                    <p className="font-medium">{order.shipping_address?.name}</p>
                    <p className="text-sm text-gray-600 mt-1">
                        {order.shipping_address?.line1}<br />
                        {order.shipping_address?.line2 && <>{order.shipping_address?.line2}<br /></>}
                ...
                <table className="w-full text-left">
                    <thead>
                        <tr className="border-b-2 border-gray-100">
                            ...
                            <th className="py-3 text-xs font-bold uppercase tracking-wider text-gray-500 text-right">Qty</th>
                        ...
                        {order.items.map((item: any, index: number) => (
                            <tr key={index} className="border-b border-gray-50">
                                <td className="py-4">
                                    ...
                                    <p className="text-gray-500 text-xs">SKU: {item.sku || 'N/A'}</p>
                                </td>
            ...
            <div className="border-t border-gray-100 pt-8 text-center text-sm text-gray-500">
                <p className="mb-2">Thank you for choosing Vishwa Lifestyle for your spiritual journey.</p>
