...
import { AccountPageSkeleton } from "@/components/AccountSkeleton";
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      try {
        ...
        // Try to fetch orders (if table exists)
        try {
          ...
          log.debug("Orders table not available yet");
        }
        ...
        try {
          ...
          if (!wishlistError && count !== null) {
            setWishlistCount(count);
          }
        } catch (e) {
          // Wishlist table might not exist yet
          log.debug("Wishlist table not available yet");
        }

      } catch (error) {
        log.error("Error fetching user", error);
        toast.error("Unable to load account. Please refresh the page.");
      } finally {
        setLoading(false);
      }
    };
