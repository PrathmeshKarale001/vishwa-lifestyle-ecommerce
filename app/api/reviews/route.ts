...
export async function POST(request: NextRequest) {
  try {
    ...
    if (ordersError) {
      console.error(
        "Error fetching orders for review validation:",
        ordersError,
      );
    }
    ...
    if (!hasPurchased) {
      console.log(
        "Purchase validation failed for user:",
        user.id,
        "productId:",
        productId,
      );
    ...
    if (
      ...
      (error.message?.includes(
        'column "status" of relation "reviews" does not exist',
      ) ||
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!id || !status) {
      return NextResponse.json(
        { error: "Review ID and status required" },
        { status: 400 },
      );
    }
